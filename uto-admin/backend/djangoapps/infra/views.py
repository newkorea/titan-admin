...
ESXi / KVM 호스트 관리, VM 목록·전원제어, 상태 동기화, 실시간 트래픽 모니터링
"""
import json
import subprocess
import threading
...
# ──────────────────────────────────────────────
#  SSH 키 인증 기반 명령 실행 (ESXi FIPS 호환)
# ──────────────────────────────────────────────
ESXI_SSH_KEY = os.path.expanduser('~/.ssh/id_rsa_esxi')


def _ssh_key_cmd(ip, cmd, user='china', timeout=15):
    """SSH 키 인증으로 ESXi 명령 실행 (비밀번호 불필요)"""
    ssh = [
        'ssh', '-i', ESXI_SSH_KEY,
...
def _parse_pnic_bytes(output):
    """vsish pNic stats 출력에서 Rx/Tx Bytes, Packets 파싱"""
    ...
    for line in output.split('\n'):
        ...
        if line.startswith('Rx Bytes:'):
            ...
            rx_pkts = int(line.split(':')[1])
        elif line.startswith('Tx Packets:'):
            tx_pkts = int(line.split(':')[1])
    return {'rx_bytes': rx_bytes, 'tx_bytes': tx_bytes, 'rx_pkts': rx_pkts, 'tx_pkts': tx_pkts}


def _collect_single_host_traffic(host_ip, interval=5):
    """한 ESXi 호스트의 실시간 트래픽 측정 (interval초 간격 2회 스냅샷)"""
    # 1) 활성 pNIC 확인 + 첫 스냅샷
    cmd1 = (
        "active_nic=$(esxcfg-nics -l 2>/dev/null | grep ' Up ' | head -1 | awk '{print $1}');"
        "echo \"NIC:$active_nic\";"
        "vsish -e get /net/pNics/$active_nic/stats 2>/dev/null | head -8;"
        "echo '---UPTIME---';"
        "uptime"
    )
    out1, err1, rc1 = _ssh_key_cmd(host_ip, cmd1, timeout=15)
    if rc1 != 0:
        return {'ip': host_ip, 'error': err1 or 'ssh failed', 'status': 'offline'}
    ...
    for l in lines1:
        if l.startswith('NIC:'):
            nic_name = l.split(':')[1].strip()
            break
    if not nic_name:
        return {'ip': host_ip, 'error': 'no active NIC', 'status': 'error'}

    snap1 = _parse_pnic_bytes(out1)
    ...
    # 2) interval초 후 두 번째 스냅샷
    time.sleep(interval)
    cmd2 = f"vsish -e get /net/pNics/{nic_name}/stats 2>/dev/null | head -8"
    out2, err2, rc2 = _ssh_key_cmd(host_ip, cmd2, timeout=15)
    if rc2 != 0:
        return {'ip': host_ip, 'error': 'second snapshot failed', 'status': 'error'}

    snap2 = _parse_pnic_bytes(out2)

    # 3) Mbps 계산
    rx_diff = snap2['rx_bytes'] - snap1['rx_bytes']
    tx_diff = snap2['tx_bytes'] - snap1['tx_bytes']
    rx_mbps = round(rx_diff * 8 / interval / 1_000_000, 1)
    tx_mbps = round(tx_diff * 8 / interval / 1_000_000, 1)
    rx_pps = round((snap2['rx_pkts'] - snap1['rx_pkts']) / interval)
    tx_pps = round((snap2['tx_pkts'] - snap1['tx_pkts']) / interval)
    ...
    return {
        ...
        'rx_mbps': rx_mbps,
        'tx_mbps': tx_mbps,
        'total_mbps': round(rx_mbps + tx_mbps, 1),
        'rx_pps': rx_pps,
        'tx_pps': tx_pps,
        'rx_bytes_cumulative': snap2['rx_bytes'],
        'tx_bytes_cumulative': snap2['tx_bytes'],
        'uptime': uptime_str,
        'interval': interval,
    }
...
# ──────────────────────────────────────────────
#  API: 실시간 트래픽 (전체 ESXi 호스트)
# ──────────────────────────────────────────────
@allow_admin
def api_read_esxi_traffic(request):
    """모든 활성 ESXi 호스트의 실시간 트래픽 수집 (병렬)"""
    interval = int(request.GET.get('interval', 5))
    interval = max(3, min(interval, 15))  # 3~15초 제한

    cursor = connections['default'].cursor()
    cursor.execute("""
        SELECT id, name, ip_address, host_type
        FROM tbl_hypervisor
        WHERE is_active=1 AND host_type='esxi'
        ORDER BY name
    """)
    hosts = dictfetchall(cursor)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        future_map = {}
        for h in hosts:
            f = executor.submit(_collect_single_host_traffic, h['ip_address'], interval)
            future_map[f] = h

        for f in concurrent.futures.as_completed(future_map):
            h = future_map[f]
            try:
                data = f.result(timeout=interval + 20)
            except Exception as e:
                data = {'ip': h['ip_address'], 'error': str(e), 'status': 'error'}
            data['host_id'] = h['id']
            data['host_name'] = h['name']
            results.append(data)

    # 이름순 정렬
    results.sort(key=lambda x: x.get('host_name', ''))
...
# ──────────────────────────────────────────────
#  API: 단일 ESXi 호스트 상세 트래픽 (VM별)
# ──────────────────────────────────────────────
@allow_admin
def api_read_esxi_traffic_detail(request):
    """특정 ESXi 호스트의 VM별 실시간 트래픽"""
    host_id = request.GET.get('host_id')
    if not host_id:
        return JsonResponse({'result': 400, 'message': 'host_id required'})

    cursor = connections['default'].cursor()
    host = _get_host(cursor, host_id)
    if not host:
        return JsonResponse({'result': 404, 'message': 'host not found'})

    ip = host['ip_address']

    # vSwitch0 포트 목록 가져오기
    cmd_ports = "vsish -e ls /net/portsets/vSwitch0/ports/ 2>/dev/null"
    out_ports, _, rc = _ssh_key_cmd(ip, cmd_ports, timeout=15)
    if rc != 0:
        return JsonResponse({'result': 500, 'message': 'failed to list ports'})

    port_ids = [p.strip().rstrip('/') for p in out_ports.strip().split('\n') if p.strip()]

    # 각 포트의 이름과 clientStats 수집 (BusyBox: for-loop)
    port_list = ' '.join(port_ids)
    cmd_stats = (
        f"for p in {port_list}; do "
        f"name=$(vsish -e get /net/portsets/vSwitch0/ports/$p/status 2>/dev/null | grep clientName | sed 's/.*clientName://'); "
        f"cs=$(vsish -e get /net/portsets/vSwitch0/ports/$p/clientStats 2>/dev/null); "
        f"txb=$(echo \"$cs\" | grep 'bytesTxOK' | head -1 | sed 's/.*://'); "
        f"rxb=$(echo \"$cs\" | grep 'bytesRxOK' | head -1 | sed 's/.*://'); "
        f"txp=$(echo \"$cs\" | grep 'pktsTxOK' | head -1 | sed 's/.*://'); "
        f"rxp=$(echo \"$cs\" | grep 'pktsRxOK' | head -1 | sed 's/.*://'); "
        f"echo \"$p|$name|$txb|$rxb|$txp|$rxp\"; done"
    )

    # 스냅샷 1
    out1, _, rc1 = _ssh_key_cmd(ip, cmd_stats, timeout=120)
    if rc1 != 0:
        return JsonResponse({'result': 500, 'message': 'snapshot 1 failed'})

    interval = 5
    time.sleep(interval)

    # 스냅샷 2
    out2, _, rc2 = _ssh_key_cmd(ip, cmd_stats, timeout=120)
    if rc2 != 0:
        return JsonResponse({'result': 500, 'message': 'snapshot 2 failed'})

    def _parse_port_snap(raw):
        data = {}
        for line in raw.strip().split('\n'):
            ...
            if len(parts) >= 6:
                ...
                name = parts[1].strip()
                try:
                    tx = int(parts[2].strip()) if parts[2].strip() else 0
                    rx = int(parts[3].strip()) if parts[3].strip() else 0
                    txp = int(parts[4].strip()) if parts[4].strip() else 0
                    rxp = int(parts[5].strip()) if parts[5].strip() else 0
                except ValueError:
                    tx = rx = txp = rxp = 0
                data[pid] = {'name': name, 'tx': tx, 'rx': rx, 'txp': txp, 'rxp': rxp}
        return data

    s1 = _parse_port_snap(out1)
    s2 = _parse_port_snap(out2)

    vm_traffic = []
    skip_names = {'Management', 'vmk0', 'Shadow'}
    for pid, d2 in s2.items():
        name = d2['name']
        if not name or any(sk in name for sk in skip_names) or name.startswith('vmnic'):
            continue
        d1 = s1.get(pid, {'tx': 0, 'rx': 0, 'txp': 0, 'rxp': 0})
        tx_diff = d2['tx'] - d1.get('tx', 0)
        rx_diff = d2['rx'] - d1.get('rx', 0)
        tx_mbps = round(tx_diff * 8 / interval / 1_000_000, 2)
        rx_mbps = round(rx_diff * 8 / interval / 1_000_000, 2)
        vm_traffic.append({
            'port_id': pid,
            'vm_name': name,
            'tx_mbps': max(0, tx_mbps),
            'rx_mbps': max(0, rx_mbps),
            'total_mbps': round(max(0, tx_mbps) + max(0, rx_mbps), 2),
            'tx_cumulative_tb': round(d2['tx'] / (1024**4), 3),
            'rx_cumulative_tb': round(d2['rx'] / (1024**4), 3),
        })

    vm_traffic.sort(key=lambda x: x['total_mbps'], reverse=True)
    ...
    return JsonResponse({
        ...
        'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        'interval': interval,
        'vm_traffic': vm_traffic,
    })


# ──────────────────────────────────────────────
#  페이지 렌더: ESXi 트래픽 대시보드
# ──────────────────────────────────────────────
@allow_admin
def esxi_traffic(request):
    return render(request, 'admin/esxi_traffic.html')
