...
def globalpayletter_callback(request):
    ...
    tph.save()
    print('Global Payletter Record -> Called: ')
...
def payletter_callback(request):
    ...
    tph.save()
    print('National Payletter Record -> Called: ')

    return JsonResponse({"code":0, "message":""})


@csrf_exempt
def eximbay_callback(request):
