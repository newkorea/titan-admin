...
def app_android_version(request):
    ...
    with connections['default'].cursor() as cur:
            ...
            # 버전 코드가 8일 경우 업데이트 무시            
            if version_code == '8':
              version_code = '7'
            
            if ver_code == version_code and ver_name == version_name :
...
def app_osx_version(request):
    ...
    try:
        twv = TblOsxVersion.objects.get(ver_code=version_code)
        ret = {
            'ver_code': twv.ver_code,
            'remark': twv.remark
        }
...
def app_get_userinfo(request):
    if 'id' in request.session:
        ...
        res['id'] = id
        # 서비스 만료 체크
        print("session expire test => ", datetime.datetime.strptime(res['info_value'], '%Y-%m-%d %H:%M:%S')) 
        print("session expire test => ", datetime.datetime.now()) 
    ...
    if 'id' in request.session:
        ...
        with connections['default'].cursor() as cur:
                ...
                SELECT
                    ...
                    order by is_active desc ,is_status desc;
                '''
                cur.execute(sql)
                rows = dictfetchall(cur)
