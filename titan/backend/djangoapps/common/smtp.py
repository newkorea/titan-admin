...
def send_email(to_email, template, language, account_email=None):
    ...
    # The activation/reset link should target the account's login email.
    # Allow overriding via account_email when the destination mailbox differs (e.g., signup alias like user@dm.com_1 sending to user@dm.com)
    target_for_link = account_email if account_email else to_email
    enc_email = quote(aes.encrypt(before_time+'|'+target_for_link).decode('ascii'))
